...
from safepo.models.Constraint_Actor_Critic_copy import ConstraintActorCritic_copy
...
class PG_copy(PolicyGradient):
    def __init__(
        ...
        self.ac = ConstraintActorCritic_copy(
            actor_type=actor,
        ...
        if use_cost_value_function:
            self.cf_optimizer_1 = core.get_optimizer('Adam', module=self.ac.c_1, lr=vf_lr)
            self.cf_optimizer_2 = core.get_optimizer('Adam', module=self.ac.c_2, lr=vf_lr)
    ...
    def compute_loss_c_1(self, obs, ret):
        """
        computing cost loss
        ...
        Returns:
            torch.Tensor
        """
        return ((self.ac.c_1(obs) - ret) ** 2).mean()

    def compute_loss_c_2(self, obs, ret):
        """
        computing cost loss
        ...
        Returns:
            torch.Tensor
        """
        return ((self.ac.c_2(obs) - ret) ** 2).mean()
    ...
    def roll_out(self):
        ...
        o, ep_ret, ep_costs_1, ep_costs_2, ep_len = self.env.reset(), 0., 0., 0, 0.   
        ...
        for t in range(self.local_steps_per_epoch):
            a, v, cv_1, cv_2, logp = self.ac.step(
                torch.as_tensor(o, dtype=torch.float32))
            next_o, r, d, info = self.env.step(a) 

            c_1 = info.get('cost_1', 0.) 
            c_2 = info.get('cost_2', 0.)
            ...
            self.buf.store(
                obs=o, act=a, rew=r, val=v, logp=logp, cost_1=c_1, cost_val_1=cv_1, cost_2=c_1, cost_val_2=cv_2 
            )
            ...
            if terminal or epoch_ended:
                if timeout or epoch_ended:
                    _, v, cv_1, cv_2, _ = self.ac(torch.as_tensor(o, dtype=torch.float32)) 
                else:
                    v, cv_1, cv_2 = 0., 0., 0.  

                # Automatically compute GAE in buffer
                self.buf.finish_path(v, cv_1, cv_2)
    ...
    def update_cost_net_1(self, data: dict) -> None:
        ...
        assert hasattr(self, 'cf_optimizer_1')
        assert 'target_c_1' in data, f'provided keys: {data.keys()}'

        if self.use_cost_value_function:
            self.loss_c_before = self.compute_loss_c(data['obs'],
                                                     data['target_c_1']).item()
        ...
        for _ in range(self.train_v_iterations):
            ...
            for start in range(0, self.local_steps_per_epoch, mbs):
                ...
                self.cf_optimizer_1.zero_grad()
                loss_c = self.compute_loss_c(obs=data['obs'][mb_indices],
                                             ret=data['target_c_1'][mb_indices])
                loss_c.backward()
                losses.append(loss_c.item())
                # Average grads across MPI processes
                mpi_tools.mpi_avg_grads(self.ac.c_1)
                self.cf_optimizer_1.step()

        self.logger.store(**{
            'Loss/DeltaCost_1': np.mean(losses) - self.loss_c_before,
            'Loss/Cost_1': self.loss_c_before,
        })

    def update_cost_net_2(self, data: dict) -> None:
        """Some child classes require additional updates,
        e.g. Lagrangian-PPO needs Lagrange multiplier parameter."""
        # Ensure we have some key components
        assert self.use_cost_value_function
        assert hasattr(self, 'cf_optimizer_2')
        assert 'target_c_2' in data, f'provided keys: {data.keys()}'

        if self.use_cost_value_function:
            self.loss_c_before = self.compute_loss_c_1(data['obs'],
                                                     data['target_c']).item()
        ...
        for _ in range(self.train_v_iterations):
            ...
            for start in range(0, self.local_steps_per_epoch, mbs):
                ...
                self.cf_optimizer_2.zero_grad()
                loss_c = self.compute_loss_c_2(obs=data['obs'][mb_indices],
                                             ret=data['target_c_2'][mb_indices])
                loss_c.backward()
                losses.append(loss_c.item())
                # Average grads across MPI processes
                mpi_tools.mpi_avg_grads(self.ac.c_2)
                self.cf_optimizer_2.step()

        self.logger.store(**{
            'Loss/DeltaCost_2': np.mean(losses) - self.loss_c_before,
            'Loss/Cost_2': self.loss_c_before,
        })
