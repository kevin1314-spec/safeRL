...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    DEFAULT = {
        ...
        'reward_orientation': True,  # Reward for being upright
    ...
    def cost(self):
        ...
        vel=self.world.robot_vel()
        yaw = self.get_robot_yaw
        cost['cost'] = np.sqrt(vel[0] ** 2 + vel[1] ** 2) + yaw
        cost['cost_1'] = np.sqrt(vel[0] ** 2 + vel[1] ** 2)
        cost['cost_2'] = yaw
    ...
    def get_robot_yaw(self):
        quat = self.data.get_body_xquat(self.reward_orientation_body)
        a, b, c, d = quat
        yaw = np.arctan(2*(a*b+c*d)/(1-2*(b*2+c*2)))
        return yaw
